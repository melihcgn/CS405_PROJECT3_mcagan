...
void main()
{
	...
	//simple diffuse light implementation
	diff = max(dot(normal, lightdir), 0.0);

	//specular light implementation
	vec3 viewDir = normalize(-vPosition); // We are looking along the z axis
	vec3 reflectDir = reflect(-lightdir, normal); // Compute reflection vector
	spec = pow(max(dot(viewDir, reflectDir), 0.0), 128.0); // Compute specular component
